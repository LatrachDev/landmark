                ...
                <div className="grid grid-cols-1 mt-10 md:mt-26 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8 lg:gap-10 items-stretch">
                    {packages.map((pkg) => {
                        ...
                        return (
                            <div
                                key={pkg.id}
                                className={`relative my-5 h-full flex flex-col bg-white rounded-lg sm:rounded-xl shadow-lg sm:shadow-xl transition-all duration-300 hover:shadow-2xl ${isPopular
                            ...
                            >
                                ...
                                {isPopular && (
                                    <div
                                        ...
                                        style={{ transform: 'rotate(12deg) translateX(0.5rem) translateY(-0.5rem)' }}
                                    >
                                        <div
                                            ...
                                            style={{ fontFamily: 'var(--font-madani-bold)', transform: 'translate(2px, 2px)', zIndex: 1 }}
                                        >
                                            الأكثر طلبا
                                        </div>
                                        <div
                                            ...
                                            style={{ fontFamily: 'var(--font-madani-bold)', zIndex: 2 }}
                                        >
                                            الأكثر طلبا
                                        </div>
                                    </div>
                                ...
                                {/* Price Section */}
                                <div className={`${priceColor} px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5 ${isPopular ? 'lg:py-8' : 'lg:py-6'}`}>
                                ...
                                {/* Features List */}
                                <div className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5 lg:py-6 bg-white flex-grow">
                                    <ul className="space-y-2 sm:space-y-3 md:space-y-4">
                                        {pkg.features.map((feature, index) => (
                                            <li key={index} className="flex items-start gap-2 sm:gap-3">
                                                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-cyan-400 flex items-center justify-center flex-shrink-0 mt-0.5">
                                                    <svg
                                ...
                                {/* CTA Button */}
                                <div className={`px-3 sm:px-4 md:px-6 pb-3 sm:pb-4 md:pb-5 ${isPopular ? 'lg:pb-10' : 'lg:pb-6'}`}>
                                    <button
                                        onClick={() => {
                                            const priceWithoutComma = pkg.price.replace(/,/g, '');
                                            const videoCount = pkg.name === '1 VIDÉO' ? '1' : pkg.name === '4 VIDÉO' ? '4' : '6';
