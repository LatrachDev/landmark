...
export default async function ServicesPage() {
    ...
    return (
        <section className="font-['Jost'] relative min-h-screen">
            <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJsonLd(servicesStructuredData) }} />
            ...
            <section className="text-left py-16 px-4 sm:px-10 w-[90%] m-auto relative z-10">
                ...
                <div className="mb-20">
                    ...
                    {(['A', 'B', 'C'] as ServiceCategory[]).map((cat) => (
                        grouped[cat].length > 0 && (
                            <div key={cat} className="mb-28">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                                    {grouped[cat].map((s) => (
                                        <ServiceCard key={s.id} id={s.id} title={s.title} description={s.description} imageUrl={s.imageUrl} />
                                    ))}
                                </div>
                            </div>
                        )
