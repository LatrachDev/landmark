			...
			<div className="max-w-7xl mx-auto px-6 lg:px-10 py-12">
				...
				<div className="mb-12">
					...
					<p className="text-gray-500 font-medium">
						Consultez et gérez les messages envoyés depuis le formulaire de
						contact.
					</p>
				</div>
							...
							{messages.data && messages.data.length > 0 ? (
								messages.data.map((msg: any) => (
									<div
										...
										className={`p-6 rounded-4xl border transition-all cursor-pointer ${selectedMessage?.id === msg.id ? "bg-[#445EF2] border-[#445EF2] text-white shadow-xl shadow-[#445EF2]/20" : "bg-white border-gray-50 text-[#010E26] hover:border-[#445EF2]/30 hover:shadow-lg"}`}
									>
										<div className="flex justify-between items-start mb-2">
											<h3 className="font-bold uppercase text-sm truncate pr-4">
												{msg.full_name}
											</h3>
											<span
												className={`text-[9px] font-black uppercase px-2 py-1 rounded-lg ${selectedMessage?.id === msg.id ? "bg-white/20 text-white" : "bg-gray-50 text-gray-400"}`}
											>
												{new Date(msg.created_at).toLocaleDateString()}
											</span>
										</div>
										<p
											className={`text-xs font-medium line-clamp-1 ${selectedMessage?.id === msg.id ? "text-white/70" : "text-gray-400"}`}
										>
											{msg.message}
										</p>
									</div>
								...
								<div className="p-10 text-center bg-white rounded-4xl border border-gray-50">
								...
								{selectedMessage ? (
									...
									>
										<div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
											<div>
												<span className="text-[#445EF2] font-black uppercase text-[10px] tracking-[0.3em] mb-2 block">
													Détails du message
												</span>
												<h2 className="text-3xl font-black text-[#010E26] uppercase font-bodoni">
											...
											<div className="flex gap-4">
												<a
													...
													className="w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center text-[#010E26] hover:bg-[#445EF2] hover:text-white transition-all shadow-sm"
												>
													<svg
														className="w-5 h-5"
														fill="none"
										...
										<div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
											<div className="space-y-1">
												...
												<p className="text-[#010E26] font-bold">
													{selectedMessage.full_name}
												</p>
												<p className="text-gray-500 font-medium">
												...
												{selectedMessage.company_name && (
													<p className="text-[#445EF2] font-black uppercase text-[10px] italic mt-2">
														@ {selectedMessage.company_name}
													</p>
												)}
											</div>
												...
												<div className="flex flex-wrap gap-2">
													{selectedMessage.interests
														...
														.map((interest: string, idx: number) => (
															<span
																key={idx}
																className="bg-gray-100 text-[#010E26] px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider"
															>
																{interest.trim()}
															</span>
														))}
									...
									<div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-gray-100 rounded-[3rem] p-20 text-center opacity-50">
										...
										<p className="text-gray-300 font-black uppercase tracking-[0.3em] text-[10px]">
											Sélectionnez un message pour l'étendre
										</p>
									</div>
								)}
							</AnimatePresence>
