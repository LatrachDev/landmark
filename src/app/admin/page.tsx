...
export default function Dashboard() {
				...
				<div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 md:py-24 mt-24">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
						{[...Array(6)].map((_, index) => (
							<div
								key={index}
								className="bg-white rounded-4xl p-8 border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] h-64"
							>
	...
	return (
		<div className="font-jost bg-[#fcfdfe] min-h-screen pb-20 overflow-x-hidden">
			...
			<nav className="border-b border-gray-100 fixed top-0 left-0 right-0 z-50 backdrop-blur-sm bg-white/95">
				<div className="max-w-7xl mx-auto px-6 lg:px-10 h-24 flex justify-between items-center">
					<Link href="/admin">
						<Image
							src={mainLogo}
			...
			<div className="max-w-7xl mx-auto px-6 lg:px-10 py-16 md:py-24 mt-24">
				<motion.div
					...
					className="mb-16"
				>
					<span className="text-[#445EF2] font-black tracking-[0.3em] uppercase text-xs mb-4 block">
					...
					<p className="text-gray-500 text-lg md:text-xl max-w-2xl font-medium leading-relaxed">
						Gérez facilement votre contenu, votre équipe, vos projets et
						optimisez la présence digitale de Landmark Agency.
					</p>
				</motion.div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
					{cards.map((card, index) => (
						<motion.div
							...
							className="bg-white rounded-4xl p-8 border border-gray-100 shadow-[0_10px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.08)] transition-all cursor-pointer group flex flex-col h-full"
						>
							<div
								className={`w-16 h-16 ${card.color} rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500`}
							>
								{card.icon}
							</div>
							...
							<p className="text-gray-500 font-medium leading-relaxed grow">
								{card.desc}
							</p>
			...
			{/* Footer decoration */}
			<div className="fixed bottom-0 left-0 w-full h-1 bg-linear-to-r from-transparent via-[#445EF2]/20 to-transparent pointer-events-none" />
		</div>
	);
}
