...
const Waveform = memo(function Waveform({ isPlaying, progress }: WaveformProps): JSX.Element {
    const bars = 20;
    const barHeights = [40, 60, 30, 70, 50, 80, 35, 65, 45, 75, 55, 85, 40, 60, 50, 70, 45, 65, 55, 75];
                ...
                return (
                    <div
                        ...
                        style={{
                            ...
                            minHeight: '8px',
                            maxHeight: '100%',
                            animation: isPlaying ? `waveform ${0.5 + index * 0.1}s ease-in-out infinite` : 'none',
                        }}
                    />
                );
            })}
        ...
        <div className="bg-white rounded-lg p-4 sm:p-6 shadow-lg">
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white flex items-center justify-center flex-shrink-0 overflow-hidden p-1.5 sm:p-2">
                        <Image
                            src={audio.logo}
                            alt={audio.brandName}
                    ...
                    <div>
                        <h3 className="text-base sm:text-lg font-bold text-gray-800" style={{ fontFamily: 'Jost' }}>
                            {audio.brandName}
                        </h3>
                        <p className="text-xs sm:text-sm text-gray-500" style={{ fontFamily: 'Jost' }}>
                            {audio.personName}
                        </p>
                    </div>
                </div>
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-cyan-400 hover:bg-cyan-500 flex items-center justify-center transition-colors flex-shrink-0"
                >
                    {isCurrentlyPlaying ? (
...
const AudioOffre = memo(function AudioOffre(): JSX.Element {
    ...
    const audioTestimonials = useMemo<AudioTestimonial[]>(() => [
        { id: 1, src: Audio2, brandName: 'COSMARK', personName: 'Mme. Meryam', logo: CosmarkLogo },
        { id: 2, src: Audio1, brandName: 'Brimo Chicken', personName: 'M. Ridoine', logo: BrimoLogo },
        { id: 3, src: Audio3, brandName: 'Dar Turkia', personName: 'M. Bouchta', logo: DarTurkiaLogo },
    ], []);
