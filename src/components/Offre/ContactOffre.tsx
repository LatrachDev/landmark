...
const ContactOffre = memo(function ContactOffre(): JSX.Element {
	...
	const serviceOptions = useMemo<ServiceOption[]>(
		() => [
			...
			{
				value: "4 videos 2800dh",
				label: "4 vidéos à 2800 Dh",
				color: "#445EF2",
			},
			{
				value: "6 videos 3900dh",
				label: "6 videos à 3900 Dh",
				color: "#263973",
			},
	...
	const checkAndSetPackage = useCallback(
		(openDropdown = false): boolean => {
			...
			if (storedPackage) {
				try {
					...
					const serviceValue: string =
						packageInfo.serviceValue || packageInfo.displayText;
					if (serviceValue) {
						setFormData((prev) => {
	...
	return (
		<div
			id="contact-offre"
			className="w-full py-12 sm:py-16 md:py-20 px-4 sm:px-6 md:px-8 lg:px-12 relative overflow-hidden"
		>
			<div className="container mx-auto max-w-7xl relative z-10">
				{/* Header */}
				<div
					className={`text-center mb-8 sm:mb-12 md:mb-16 transition-all duration-500 ${showSuccessScreen ? "opacity-0 h-0 mb-0 overflow-hidden" : "opacity-100 h-auto"}`}
				>
					<h2
				...
				{!showSuccessScreen && (
					<div className="grid grid-cols-1 lg:grid-cols-2 text-right gap-8 sm:gap-10 md:gap-12 lg:gap-16 mb-12 sm:mb-16 md:mb-20 items-center">
						...
						{/* Form */}
						<div className="order-1 lg:order-2 relative min-h-100">
							<div className="bg-[#0a1a3a] rounded-lg p-6 sm:p-8 md:p-10 lg:p-12 relative">
								<p
									...
									style={{ fontFamily: "var(--font-jenine)" }}
								>
									كتب لينا شنو محتاج وغادي نخدمو معاك على خطة واضحة لصناعة
									المحتوى
								</p>
									...
									<div ref={dropdownRef} className="relative">
										...
										>
											<div className="flex items-center gap-3 flex-1 justify-end">
												{formData.service_type &&
													(() => {
														const selected = serviceOptions.find(
															(opt) => opt.value === formData.service_type,
														);
														return selected ? (
															<svg
												...
												<span
													className={
														!formData.service_type ? "text-gray-400" : ""
													}
												>
													{formData.service_type
														? serviceOptions.find(
											...
											<svg
												className={`w-5 h-5 transition-transform shrink-0 ${isDropdownOpen ? "rotate-180" : ""}`}
												fill="none"
												stroke="currentColor"
												viewBox="0 0 24 24"
											>
							...
							<div className="bg-[#0a1a3a] rounded-lg p-6 sm:p-8 md:p-10 lg:p-12 relative">
								<div className="flex flex-col items-center justify-center text-center space-y-6 sm:space-y-8">
									<div className="mx-auto mb-6 w-20 h-20 sm:w-24 sm:h-24 bg-green-500 rounded-full flex items-center justify-center animate-bounce-in">
										<svg
											className="w-12 h-12 sm:w-16 sm:h-16 text-white animate-check-draw"
											fill="none"
											stroke="currentColor"
											strokeWidth={2}
											viewBox="0 0 24 24"
									...
									<div className="space-y-4">
										<h3
											className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-green-400 animate-slide-up"
											style={{ fontFamily: "var(--font-jenine)" }}
										>
											!شكراً لتواصلك معنا
										</h3>
										<p
											className="text-white text-lg sm:text-xl md:text-2xl animate-slide-up-delay"
											style={{ fontFamily: "var(--font-madani-regular)" }}
										>
											تم إرسال طلبك بنجاح
										</p>
										<p
											className="text-gray-300 text-base sm:text-lg animate-slide-up-delay"
											style={{ fontFamily: "var(--font-madani-regular)" }}
										>
				...
				{/* Contact Info */}
				<div
					className={`grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 md:gap-10 transition-all duration-500 ${showSuccessScreen ? "opacity-0 h-0 overflow-hidden" : "opacity-100 h-auto"}`}
				>
					<div className="flex flex-col items-center text-center">
						...
						<a
							href="mailto:contanct.landmarkagency@gmail.com"
							className="text-white text-sm sm:text-base hover:text-cyan-400 transition-colors"
							style={{ fontFamily: "var(--font-madani-regular)" }}
						>
