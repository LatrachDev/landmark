...
const Nav = memo(() => {
	...
	return (
		<>
			<nav
				className={`w-full sticky top-0 font-jost z-50 ${
					!isHome
						? "bg-white"
						: scrolled
							? "bg-white shadow-sm"
				...
				<div className="w-[90%] m-auto">
					<div className="flex justify-between items-center">
						...
						<Link
							...
							aria-label="Landmark - Retour à l'accueil"
						>
							<img
								src={
									shouldUseDarkColors || menuOpen ? mainLogo.src : whiteLogo.src
								}
			...
			{menuVisible && (
				<div
					id="mobile-menu"
					className={`xl:hidden fixed inset-0 z-40 pt-20 font-jost bg-white transform transition-transform duration-500 ease-in-out ${
						menuAnimating ? "translate-y-0" : "-translate-y-full"
					}`}
					role="dialog"
					aria-modal="true"
						...
						<div className="flex flex-col space-y-8 text-center">
							{navLinks.map((link, index) => (
								<Link
									key={link.path}
									href={link.path}
									className={`text-xl sm:text-2xl relative block group text-black hover:text-[#445EF2] ${
										pathname === link.path ? "font-semibold" : ""
									} transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#445EF2] focus:ring-offset-2 rounded ${
										menuAnimating
											? "opacity-100 translate-y-0"
											: "opacity-0 -translate-y-4"
									}`}
									style={{
										transitionDelay: menuAnimating
											? `${150 + index * 60}ms`
											: "0ms",
							...
							<Link
								href="/contact"
								className={`bg-transparent text-black border-black border text-sm sm:text-xl px-6 py-3 transition-all duration-500 hover:bg-[#445EF2] hover:border-[#445EF2] hover:text-white mt-8 focus:outline-none focus:ring-2 focus:ring-[#445EF2] focus:ring-offset-2 rounded ${
									menuAnimating
										? "opacity-100 translate-y-0"
										: "opacity-0 translate-y-4"
								}`}
